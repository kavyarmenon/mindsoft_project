...
@Component({
  selector: "app-edit-settlement-policy",
  templateUrl: "./edit-settlement-policy.component.html",
  styleUrls: ["./edit-settlement-policy.component.css"],
})
export class EditSettlementPolicyComponent implements OnInit {
  ...
  policyName: "";
  gratuityPolicyId = localStorage.getItem("gratuityPolicyId");
  ...
  getEditData() {
    this.serverService
      .getData(
        "api/GratuityPolicyAPI/FindGratuityPolicyById?SetPolicyID=" +
          this.gratuityPolicyId
      )
      .subscribe((res) => {
        this.policyName = res["gratuityMaster"]["description"];
        this.calcDayNormal = res["gratuityMaster"]["calculationIdNormal"];
        this.isIncludeEligibleLeaveNormal =
          res["gratuityMaster"]["isIncludeLeaveNormal"];
        this.calcBasedOnNormal =
          res["gratuityMaster"]["isCompanyBasedOnNormal"];
        this.calcDayTermination =
          res["gratuityMaster"]["calculationIdTerminal"];
        this.isIncludeEligibleLeaveTermination =
          res["gratuityMaster"]["isIncludeLeaveterminal"];
        this.calcBasedOnTermination =
          res["gratuityMaster"]["isCompanyBasedOnTerminal"];
        //  Normal
        res["gratuityPolicyList"].forEach((element) => {
          let dctData = {};
          dctData["parameter"] = element.particularsId;
          dctData["noOfMonth"] = element.noOfMonths;
          dctData["noOfDays"] = element.noOfDays;
          this.lstLessThanNormal.push(dctData);
        });

        res["gratuityPolicyFiveYr"].forEach((element) => {
          let dctData = {};
          dctData["parameter"] = element.particularsId;
          dctData["noOfMonth"] = element.noOfMonths;
          dctData["noOfDays"] = element.noOfDays;
          this.lstGreaterThanNormal.push(dctData);
        });

        // termination

        res["gratuityPolicyTer"].forEach((element) => {
          let dctData = {};
          dctData["parameter"] = element.particularsId;
          dctData["noOfMonth"] = element.noOfMonths;
          dctData["noOfDays"] = element.noOfDays;
          this.lstLessThanTermination.push(dctData);
        });

        res["gratuityPolicyFiveYrTer"].forEach((element) => {
          let dctData = {};
          dctData["parameter"] = element.particularsId;
          dctData["noOfMonth"] = element.noOfMonths;
          dctData["noOfDays"] = element.noOfDays;
          this.lstGreaterThanTermination.push(dctData);
        });
      });
  }
  ...
  saveUserDetails() {
    ...
    let dctSaveDetails = {
      Status: "",
      gratuityMaster: {
        gratuityPolicyId: this.gratuityPolicyId,
